from app.split_integer import split_integer


def test_sum_of_the_parts_should_be_equal_to_value() -> None:
    assert sum(split_integer(10, 2)) == 10


def test_sum_of_the_parts_should_be_equal_to_value_when_not_divisible() -> None:
    assert sum(split_integer(10, 4)) == 10


def test_should_split_into_equal_parts_when_value_divisible_by_parts() -> None:
    assert split_integer(10, 2) == [5, 5]
    assert all(part == 5 for part in split_integer(10, 2))


def test_should_return_part_equals_to_value_when_split_into_one_part() -> None:
    assert split_integer(10, 1) == [10]


def test_parts_should_be_sorted_when_they_are_not_equal() -> None:
    assert split_integer(10, 3) == sorted(split_integer(10, 3))
    assert len(set(split_integer(10, 3))) > 1

def test_should_add_zeros_when_value_is_less_than_number_of_parts() -> None:
    assert split_integer(1, 3) == [0, 0, 1]


def test_length_of_result_should_equal_parts() -> None:
    assert len(split_integer(10, 4)) == 4
    assert len(split_integer(1, 3)) == 3
    assert len(split_integer(100, 1)) == 1


def test_difference_between_max_and_min_should_be_at_most_one() -> None:
    assert max(split_integer(10, 4)) - min(split_integer(10, 4)) <= 1
    assert max(split_integer(1, 3)) - min(split_integer(1, 3)) <= 1
    assert max(split_integer(9, 2)) - min(split_integer(9, 2)) <= 1
